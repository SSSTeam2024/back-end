...
const updateVehicleAffiliateById = async (req, res) => {
  try {
    ...
    if (documents[0].base64String != undefined) {
      vehicleBody.vehicle_images = documents[0].name;
    }
    if (documents[1].base64String != undefined) {
      vehicleBody.mot_file = documents[1].name;
    }
    if (documents[2].base64String != undefined) {
      vehicleBody.tax_file = documents[2].name;
    }
    if (documents[3].base64String != undefined) {
      vehicleBody.insurance_file = documents[3].name;
    }

    const vehicle = await vehicleAffiliateService.updateVehicleAffiliate(
      vehicleAffiliateId,
      vehicleBody,
      documents
    );
